...
export class AnalyticsComponent {
  ...
  async embedReport(): Promise<void> {
    ...
    const displayMessage =
      this.element.nativeElement.querySelector('.display-message');
    if (displayMessage) {
    ...
    this.displayMessage =
      'Access token is successfully set. Loading Power BI report.';
  }
   ...
   * @returns Promise<void>
   */
  async prepareDisplayMessageForEmbed(
  ...
  async deleteVisual(): Promise<void> {
    ...
    if (activePage) {
      ...
      const visual: VisualDescriptor | undefined = visuals.find(
        (v) =>
